...
function App() {
  ...
  const searchTweets = (query) => {
    ...
    const endpoint = `https://api.twitter.com/2/tweets/search/recent?query=${encodeURIComponent(query)}&tweet.fields=created_at`;
    
    // Bearer token (should be securely stored in production)
